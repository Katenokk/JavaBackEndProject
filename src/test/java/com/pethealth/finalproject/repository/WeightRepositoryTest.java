...
//        private TestEntityManager entityManager;
        ...
        public void findAllBetweenDates_ReturnsCorrectWeights() {
            ...
            assertTrue(weights.contains(weight1));
            assertTrue(weights.contains(weight2));
        }

        @Test
        void findAllByHealthRecordAndDayBetween_ReturnsCorrectWeights() {
            Owner owner2 = new Owner("Catto Owner", "new-owner", "1234", new ArrayList<>(), "owner@mail.com");
            userRepository.save(owner2);
            LocalDate dateOfBirth = LocalDate.of(200, 1, 1);
            Date dateOfBirthOld = Date.from(dateOfBirth.atStartOfDay().toInstant(java.time.ZoneOffset.UTC));
            Cat cat = new Cat("Catto", dateOfBirthOld, false, List.of(CatDiseases.IBD), CatBreeds.BENGAL, owner2, null);
            petRepository.save(cat);

            HealthRecord healthRecord1 = new HealthRecord(cat);
            healthRecordRepository.save(healthRecord1);
            ...
            List<Weight> weights = weightRepository.findAllByHealthRecordAndDayBetween(healthRecord1, startDate, endDate);
